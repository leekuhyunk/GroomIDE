...
void setup() {
  ...
  Serial.println("IR input Strarting");
}

void loop() {
  // 전원ON시 무한반복
  if(irrecv.decode(&results)) { // 수신값을 확인해서 있으면 true 반환하는 함수 decodee
    ...
    //switch문 end
    irrecv.resume();//ready 수신준비 상태로 만들기 다음 수신값을 받는다
  }
}